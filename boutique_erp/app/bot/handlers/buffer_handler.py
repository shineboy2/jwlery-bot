"""
Buffer management handler — view and manage buffered posts per category.
"""
import logging
...
from app.database.crud import (
    ...
    get_buffer_by_category, get_post_by_id,
    move_post_to_queue, delete_content_post,
    count_buffered_by_category, update_post_content,
    SYSTEM_CATEGORIES,
)
from app.bot.keyboards.inline import (
    build_kb, cms_buffer_menu_keyboard, cms_buffer_list_keyboard,
    cms_buffer_post_keyboard,
)
from app.services.cms_service import publish_post_to_channel

logger = logging.getLogger(__name__)

# FSM states for inline editing in buffer
_buffer_edit_states = {}  # {chat_id: {"post_id": int, "action": "edit_cap"|"edit_img", "cat_code": str}}


async def callback_cms_buffer_menu(callback: CallbackQuery):
    """Show buffer menu with counts per category."""
    ...
    for code in SYSTEM_CATEGORIES:
        counts[code] = await count_buffered_by_category(code)
    for cat in custom_cats:
        counts[cat.code] = await count_buffered_by_category(cat.code)
...
async def callback_cms_buffer_category(callback: CallbackQuery, cat_code: str):
    """Show list of buffered posts in a category."""
    chat_id = callback.message.chat.id
    admin = await get_admin_by_chat_id(chat_id)
    if not admin or not admin.is_active:
        return

    posts = await get_buffer_by_category(cat_code)
    ...
    await callback.message.edit(
        text,
        components=cms_buffer_list_keyboard(posts, cat_code)
    )


async def callback_cms_buffer_post_detail(callback: CallbackQuery, post_id: int):
    """Show detail and actions for a single buffered post."""
    chat_id = callback.message.chat.id
    admin = await get_admin_by_chat_id(chat_id)
    if not admin or not admin.is_active:
        return
    ...
    await callback.message.edit(
        text,
        components=cms_buffer_post_keyboard(post_id, post.category)
    )


async def callback_cms_buffer_to_queue(callback: CallbackQuery, post_id: int):
    """Move a buffered post to the end of its category's queue."""
    post = await get_post_by_id(post_id)
    if not post:
        return
    cat_code = post.category
    result = await move_post_to_queue(post_id)
    if result:
        await callback.message.edit(
...
async def callback_cms_buffer_delete(callback: CallbackQuery, post_id: int):
    """Delete a buffered post."""
    post = await get_post_by_id(post_id)
    if not post:
        return
    cat_code = post.category
...
async def callback_cms_buffer_publish_now(callback: CallbackQuery, post_id: int):
    """Publish a buffered post immediately to channel."""
...
async def callback_cms_buffer_edit_cap(callback: CallbackQuery, post_id: int):
    """Start inline caption editing for a buffered post."""
    chat_id = callback.message.chat.id
    post = await get_post_by_id(post_id)
    if not post:
        return
    _buffer_edit_states[chat_id] = {"post_id": post_id, "action": "edit_cap", "cat_code": post.category}
...
async def callback_cms_buffer_edit_img(callback: CallbackQuery, post_id: int):
    """Start inline image editing for a buffered post."""
    chat_id = callback.message.chat.id
    post = await get_post_by_id(post_id)
    if not post:
        return
    _buffer_edit_states[chat_id] = {"post_id": post_id, "action": "edit_img", "cat_code": post.category}
    cancel_kb = build_kb([[InlineKeyboardButton("❌ لغو", callback_data=f"cms:buf:post:{post_id}")]])
    await callback.message.reply("🖼 عکس جدید را بفرستید:", components=cancel_kb)


async def handle_buffer_edit_message(message: Message) -> bool:
    """Handle FSM editing for buffered posts. Returns True if consumed."""
    chat_id = message.chat.id
    state = _buffer_edit_states.get(chat_id)
    if not state:
        return False
    ...
    if action == "edit_cap":
        ...
        if not caption:
            ...
            return True
        await update_post_content(post_id, caption=caption)
        _buffer_edit_states.pop(chat_id, None)
        preview_kb = build_kb([[InlineKeyboardButton("📌 نمایش پست", callback_data=f"cms:buf:post:{post_id}")]])
